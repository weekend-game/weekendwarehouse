...
public class TableModelDB extends AbstractTableModel implements ITableModel {

	public TableModelDB(TableDefinition definition, IDB db) throws Exception {
		...
		for (int i = 1; i <= met.getColumnCount(); ++i) { // Считаны поля
			for (int j = 0; j < colNum; ++j) { // Поля для отображения
				if (definition.columns.get(j).source.equalsIgnoreCase(met.getColumnName(i))) {
					fromIndex[k] = i; // номер считанного из БД поля
					toIndex[k] = j; // номер поля к отображению в определении
					cTest[j] = true; // Отметка "поле отображения нашлось"
										// А класс-то определеяется по типу поля в таблице !!!!!!
