...
public class SummaryTableRowSorter<M extends ITableModel> extends
		TableRowSorter<M> {

	public SummaryTableRowSorter(M model) {
		super(model);
	}
	...
	class SummaryModelWrapper<M extends ITableModel> extends
			DefaultRowSorter.ModelWrapper<M, Integer> {

		private DefaultRowSorter.ModelWrapper<M, Integer> delegate;

		public SummaryModelWrapper(
				DefaultRowSorter.ModelWrapper<M, Integer> delegate) {
		...
		public Object getValueAt(int row, int column) {
			return new TableCellValue(row, column, delegate.getValueAt(row,
					column));
		}
	...
	class TableCellValueComparator implements Comparator<TableCellValue> {
		...
		public TableCellValueComparator(
				@SuppressWarnings("rawtypes") Comparator delegate) {
			this.delegate = delegate;
		}
		...
		public int compare(TableCellValue cell1, TableCellValue cell2) {
			...
			if (cell1.row == getModelWrapper().getRowCount() - 1)
				...
				if (delegate != null)
					...
					String v1 = getModelWrapper().getStringValueAt(cell1.row,
							cell1.column);
					String v2 = getModelWrapper().getStringValueAt(cell2.row,
							cell2.column);
					if (v1 == null)
						return v2 == null ? 0 : -1;
					else
